...
def integration(**overrides):
    values = {
        ...
        "transaction_result_key": "result",
        "field_mapping": {},
        "client": SimpleNamespace(name="Bio Brain"),
    }
...
class ConfigurableProviderClientTests(SimpleTestCase):
    ...
    def test_biobrain_uses_exact_endpoint_header_and_normalizes_inventory(self):
        session = CapturingSession({"status": "ok", "hasError": False, "Surveys": [{
            "SurveyId": 44, "Name": "Bio study", "Revenue": 2.5, "IncidentRate": 35,
            "LengthOfInterview": 12, "SurveyUrl": "https://respond.voqall.com/l?vq_sid=44",
            "Has_Quotas": True, "LastUpdatedOnUTC": "2026-08-09T10:00:00Z",
        }]})
        client = InnovateMRClient(token="secret", session=session, integration=integration())
        surveys = client.get_allocated_surveys()
        ...
        self.assertEqual(surveys[0]["surveyId"], 44)
        self.assertEqual(surveys[0]["surveyName"], "Bio study")
        self.assertEqual(surveys[0]["CPI"], 2.5)
        self.assertEqual(surveys[0]["entryLink"], "https://respond.voqall.com/l?vq_sid=44")
    ...
    def test_biobrain_detail_endpoints_are_configurable(self):
        ...
        client = InnovateMRClient(token="secret", session=session, integration=integration())
        self.assertEqual(client.get_quota_for_survey(44)[0]["id"], 7)
        self.assertEqual(client.get_survey_targeting(44)[0]["QuestionId"], 9)
        self.assertTrue(session.calls[0][0].endswith("/survey-quotas/44"))
        self.assertTrue(session.calls[1][0].endswith("/survey-qualifications/44"))

    def test_custom_provider_field_mapping(self):
        session = CapturingSession({"data": {"items": [{"id": 8, "title": "Custom study"}]}})
        configured = integration(
            provider_code="custom", base_url="https://example.test/api", inventory_endpoint="surveys",
            auth_header_name="X-API-Key", inventory_result_key="data.items",
            field_mapping={"surveyId": "id", "surveyName": "title"},
        )
        survey = InnovateMRClient(token="secret", session=session, integration=configured).get_allocated_surveys()[0]
        self.assertEqual(session.calls[0][0], "https://example.test/api/surveys")
        self.assertEqual(survey["surveyId"], 8)
        self.assertEqual(survey["surveyName"], "Custom study")
