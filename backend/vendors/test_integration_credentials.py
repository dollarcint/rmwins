...
class ClientIntegrationCredentialTests(TestCase):
    ...
    def test_changed_key_clears_only_its_own_links(self):
        ...
        self.assertFalse(changed); self.assertEqual(cleared, 0)
        survey_a.refresh_from_db(); self.assertTrue(survey_a.entry_link)
        changed, cleared = set_integration_token(self.integration_a, "replacement-token")
        self.assertTrue(changed); self.assertEqual(cleared, 1)
        survey_a.refresh_from_db(); survey_b.refresh_from_db()
        self.assertEqual(survey_a.entry_link, ""); self.assertEqual(survey_a.raw_data, {})
