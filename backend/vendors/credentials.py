...
def set_integration_token(integration, token: str) -> tuple[bool, int]:
    ...
    integration.save(update_fields=[
        "credential_env_key", "encrypted_api_token", "credential_fingerprint", "credential_last_four",
...
@transaction.atomic
def set_integration_env_key(integration, env_key: str) -> tuple[bool, int]:
    """Use an environment variable as the only credential source for an integration."""
    env_key = env_key.strip()
    token = os.getenv(env_key, "").strip() if env_key else ""
    fingerprint = token_fingerprint(token)
    previous_fingerprint = integration.credential_fingerprint
    if not previous_fingerprint and integration.encrypted_api_token:
        previous_fingerprint = token_fingerprint(resolve_integration_token(integration))
    changed = bool(previous_fingerprint and previous_fingerprint != fingerprint)
    cleared = _clear_integration_data(integration) if changed else 0
    integration.credential_env_key = env_key
    integration.encrypted_api_token = ""
    integration.credential_fingerprint = fingerprint
    integration.credential_last_four = token[-4:] if token else ""
    integration.credential_changed_at = timezone.now()
    integration.save(update_fields=[
        "credential_env_key", "encrypted_api_token", "credential_fingerprint",
        "credential_last_four", "credential_changed_at", "updated_at",
    ])
    return changed, cleared
...
def reconcile_all_integration_credentials() -> dict[str, int]:
    ...
    for integration in ClientIntegration.objects.exclude(credential_env_key=""):
        ...
        if not integration.credential_fingerprint:
            set_integration_env_key(integration, integration.credential_env_key)
        elif integration.credential_fingerprint != fingerprint:
            was_changed, count = set_integration_env_key(integration, integration.credential_env_key)
