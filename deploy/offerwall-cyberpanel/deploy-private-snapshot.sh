#!/usr/bin/env bash
# Deploy a trusted source snapshot to the isolated Offerwall services.
#
...
# /var/lib/rmwins-offerwall-deploy/deploy-private-snapshot.sh (root:root mode 0700), then:
...
APP_DIR="$APP_HOME/offerwall"
BACKEND_DIR="$APP_DIR/backend"
VENV_DIR="$BACKEND_DIR/.venv"
CONFIG_DIR="$APP_HOME/.config/rmwins-offerwall"
BACKEND_ENV="$CONFIG_DIR/backend.env"
REDIS_PERSISTENT_CONFIG="$CONFIG_DIR/redis-6384.conf"
REDIS_CACHE_CONFIG="$CONFIG_DIR/redis-6385.conf"
ADMIN_CREDENTIALS="$CONFIG_DIR/admin-credentials"
FRONTEND_RELEASE="$APP_HOME/app/offerwall/frontend-dist"
STATE_DIR="/var/lib/rmwins-offerwall-deploy"
BACKUP_ROOT="/var/backups/rmwins-offerwall"
SNAPSHOT_ARCHIVE="${SNAPSHOT_ARCHIVE:-}"
PYTHON_BIN="${PYTHON_BIN:-/usr/bin/python3.11}"
SUPERVISOR_CONFIG="$APP_DIR/deploy/offerwall-cyberpanel/supervisord.conf"
...
fail() {
    printf '\nOFFERWALL PRIVATE DEPLOY FAILED: %s\n' "$*" >&2
    exit 1
}
...
cleanup() {
    ...
    if test -n "$STAGING_DIR" && test -d "$STAGING_DIR"; then
        case "$STAGING_DIR" in
            /var/tmp/rmwins-offerwall-stage.*) rm -rf -- "$STAGING_DIR" ;;
        esac
    fi
}
...
on_error() {
    ...
    if test "$RMWINS_SERVICES_STOPPED" -eq 1; then
        systemctl stop rmwins-offerwall-frontend.service rmwins-offerwall-supervisor.service >/dev/null 2>&1 || true
        printf 'The private Offerwall services remain stopped to avoid running mixed code/schema state.\n' >&2
    fi
}
...
for port in 6384 6385 8095 8096; do
    if ss -H -ltn | awk '{print $4}' | grep -Eq "(^|:)${port}$"; then
        case "$port" in
            6384|6385|8095) systemctl is-active --quiet rmwins-offerwall-supervisor.service \
                || fail "Port $port is occupied before the RMWins service is active." ;;
            8096) systemctl is-active --quiet rmwins-offerwall-frontend.service \
...
if command -v firewall-cmd >/dev/null 2>&1; then
    for port in 5432 6384 6385 8095 8096; do
...
RMWINS_SUPERVISOR_BEFORE="$(systemctl is-active rmwins-supervisor.service 2>/dev/null || true)"
RMWINS_FRONTEND_BEFORE="$(systemctl is-active rmwins-frontend.service 2>/dev/null || true)"

log "Validating and extracting trusted snapshot"
if tar -tzf "$SNAPSHOT_ARCHIVE" | awk '
    /^\// || /(^|\/)\.\.($|\/)/ { bad=1 }
    /(^|\/)\.env$/ || /(^|\/)(backend\.env|admin-credentials|redis-638[45]\.conf)$/ { bad=1 }
...
STAGING_DIR="$(mktemp -d /var/tmp/rmwins-offerwall-stage.XXXXXX)"
...
for trusted_file in \
    ...
    "$STAGING_DIR/deploy/offerwall-cyberpanel/backend.env.example" \
    "$STAGING_DIR/deploy/offerwall-cyberpanel/redis-persistent-6384.conf.example" \
    "$STAGING_DIR/deploy/offerwall-cyberpanel/redis-cache-6385.conf.example" \
    "$STAGING_DIR/deploy/offerwall-cyberpanel/rmwins-offerwall-supervisor.service" \
    "$STAGING_DIR/deploy/offerwall-cyberpanel/rmwins-offerwall-frontend.service"; do
...
for unit in rmwins-offerwall-frontend.service rmwins-offerwall-supervisor.service; do
...
run_app_clean mkdir -p "$APP_DIR" "$APP_HOME/app" \
    "$APP_HOME/app/offerwall/data/redis-persistent" "$APP_HOME/app/offerwall/data/redis-cache" \
    "$APP_HOME/app/offerwall/logs/frontend" "$APP_HOME/app/offerwall/logs/supervisor" \
    "$APP_HOME/app/offerwall/run/frontend" "$APP_HOME/app/offerwall/run/supervisor" \
    "$APP_HOME/app/offerwall/tmp/frontend/client_body" "$APP_HOME/app/offerwall/tmp/frontend/proxy" \
    "$APP_HOME/app/offerwall/tmp/frontend/fastcgi" "$APP_HOME/app/offerwall/tmp/frontend/uwsgi" \
    "$APP_HOME/app/offerwall/tmp/frontend/scgi"
...
if test ! -e "$BACKEND_ENV"; then
    run_root_python - "$STAGING_DIR/deploy/offerwall-cyberpanel/backend.env.example" "$BACKEND_ENV" <<'PY'
import os
import secrets
import sys
...
expected = {
    ...
    "DB_NAME": "rmwins_offerwall_prod",
    "DB_USER": "rmwins_offerwall_app",
    "DB_HOST": "127.0.0.1",
    "DB_PORT": "5432",
    "PRESCREENER_VAULT_ENABLED": "true",
    "PRESCREENER_DB_ENGINE": "postgresql",
    "PRESCREENER_DB_NAME": "rmwins_offerwall_vault",
    "PRESCREENER_DB_USER": "rmwins_offerwall_vault_app",
...
urls = {
    "CELERY_BROKER_URL": (6384, 0),
    "CELERY_RESULT_BACKEND": (6384, 1),
    "REDIS_CACHE_URL": (6385, 0),
    "PROJECTS_REDIS_CACHE_URL": (6385, 1),
}
passwords = {}
...
test "$DB_NAME:$DB_USER:$VAULT_DB_NAME:$VAULT_DB_USER" = \
    "rmwins_offerwall_prod:rmwins_offerwall_app:rmwins_offerwall_vault:rmwins_offerwall_vault_app" \
...
if test "$ADMIN_ACTIVE_SUPERUSERS" -gt 0; then
    ...
    if test ! -e "$ADMIN_CREDENTIALS"; then
        ...
        {
            printf 'username=offerwall_admin\npassword='
            openssl rand -hex 24
...
for line in Path(sys.argv[1]).read_text(encoding="utf-8").splitlines():
    if "=" in line:
        ...
        values[key] = value
if values.get("username") != "offerwall_admin" or len(values.get("password", "")) < 32:
    raise SystemExit("Invalid protected administrator credential file")
print(values["password"])
' "$ADMIN_CREDENTIAL_SOURCE" | \
    run_app_clean env ADMIN_USERNAME=offerwall_admin ADMIN_EMAIL=offerwall-admin@localhost.invalid \
...
run_app_clean /usr/sbin/nginx -t -e stderr \
    -c "$APP_DIR/deploy/offerwall-cyberpanel/frontend-nginx.conf"
...
required = {
    "program:rmwins-offerwall-redis-persistent", "program:rmwins-offerwall-redis-cache",
    "program:rmwins-offerwall-web", "program:rmwins-offerwall-worker", "program:rmwins-offerwall-beat",
}
...
systemd-analyze verify \
    "$STAGING_DIR/deploy/offerwall-cyberpanel/rmwins-offerwall-supervisor.service" \
    "$STAGING_DIR/deploy/offerwall-cyberpanel/rmwins-offerwall-frontend.service"
for unit in rmwins-offerwall-supervisor.service rmwins-offerwall-frontend.service; do
    ...
    source_unit="$STAGING_DIR/deploy/offerwall-cyberpanel/$unit"
...
systemctl enable rmwins-offerwall-supervisor.service rmwins-offerwall-frontend.service >/dev/null
restart_or_start rmwins-offerwall-supervisor.service
restart_or_start rmwins-offerwall-frontend.service
sleep 8

log "Running private health checks"
systemctl is-active --quiet rmwins-offerwall-supervisor.service || fail "RMWins Supervisor service is inactive."
systemctl is-active --quiet rmwins-offerwall-frontend.service || fail "RMWins frontend service is inactive."
...
REDISCLI_AUTH="$REDIS_PERSISTENT_PASSWORD" redis-cli -h 127.0.0.1 -p 6384 ping | grep -Fxq PONG \
    || fail "Persistent Redis health check failed."
REDISCLI_AUTH="$REDIS_CACHE_PASSWORD" redis-cli -h 127.0.0.1 -p 6385 ping | grep -Fxq PONG \
    || fail "Cache Redis health check failed."
REDISCLI_AUTH="$REDIS_CACHE_PASSWORD" redis-cli -h 127.0.0.1 -p 6385 FLUSHALL >/dev/null
...
curl --fail --silent --show-error --max-time 20 http://127.0.0.1:8096/__health | grep -Fxq ok \
    || fail "Frontend private health check failed."
curl --fail --silent --show-error --max-time 20 \
    -H 'Host: offerwall.rmwinsights.com' -H 'X-Forwarded-Proto: https' \
    http://127.0.0.1:8095/api/v1/auth/session/ >/dev/null \
...
if ss -H -ltn | awk '$4 ~ /:(6384|6385|8095|8096)$/ {print $4}' \
    | grep -Ev '^127\.0\.0\.1:(6384|6385|8095|8096)$' | grep -q .; then
...
RMWINS_SUPERVISOR_AFTER="$(systemctl is-active rmwins-supervisor.service 2>/dev/null || true)"
RMWINS_FRONTEND_AFTER="$(systemctl is-active rmwins-frontend.service 2>/dev/null || true)"
...
test "$RMWINS_SUPERVISOR_AFTER" = "$RMWINS_SUPERVISOR_BEFORE" \
    || fail "Production RMWins Supervisor state changed unexpectedly."
test "$RMWINS_FRONTEND_AFTER" = "$RMWINS_FRONTEND_BEFORE" \
    || fail "Production RMWins frontend state changed unexpectedly."
...
log "OFFERWALL PRIVATE DEPLOY SUCCESS"
printf 'Snapshot: %s\n' "$SNAPSHOT_SHA256"
printf 'Backend:  http://127.0.0.1:8095 (healthy)\n'
printf 'Frontend: http://127.0.0.1:8096 (healthy)\n'
printf 'Redis:    127.0.0.1:6384 persistent, 127.0.0.1:6385 cache\n'
